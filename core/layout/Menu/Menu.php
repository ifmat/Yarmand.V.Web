<aside class="sidebar col-md-3 col-lg-2 position-fixed end-0 top-0 min-vh-100 d-flex flex-column">

    <div class="sidebar-header text-center">
        <div class="lang-switch mb-3">
            <button id="btn-fa" class="btn btn-sm btn-outline-light">فارسی</button>
            <button id="btn-en" class="btn btn-sm btn-outline-light">English</button>
        </div>

        <div class="avatar-wrapper">
            <img src="assets/images/avatar.png" alt="Avatar">
        </div>

        <div class="welcome-text mt-3">
            <p Multi_Lang="Welcome" class="mb-1"></p>
            <span class="fw-bold">Matin</span>
        </div>
    </div>

    <nav class="menu mt-4 w-100">
        <ul class="list-unstyled w-100 px-3">

            <li>
                <a href="#" class="menu-item active">
                    <i class="fa-solid fa-house"></i>
                    <span Multi_Lang="traneh"></span>
                </a>
            </li>

            <li>
                <a href="#" class="menu-item">
                    <i class="fa-solid fa-user"></i>
                    <span Multi_Lang="item_menu"></span>
                </a>
            </li>

            <li>
                <a href="#" class="menu-item">
                    <i class="fa-solid fa-chart-line"></i>
                    <span Multi_Lang="item_menu"></span>
                </a>
            </li>

            <li>
                <a href="#" class="menu-item">
                    <i class="fa-solid fa-gear"></i>
                    <span Multi_Lang="item_menu"></span>
                </a>
            </li>

            <li>
                <a href="#" class="menu-item logout">
                    <i class="fa-solid fa-right-from-bracket"></i>
                    <span Multi_Lang="item_Delete_menu"></span>
                </a>
            </li>

        </ul>
    </nav>
</aside>
